...
class DTDataManager(Base_Cog):
  ...
  @manager.sub_command(description=Strings.event_data_manager_fetch_data_description)
  ...
  async def fetch_guild_data(self, inter: disnake.CommandInteraction, guild_id: int=commands.Param(description="Deep Town Guild ID")):
    ...
    if data is None:
      return await message_utils.generate_error_message(inter, Strings.event_data_manager_fetch_data_get_failed)

    event_participation_repo.generate_or_update_event_participations(data)

    await message_utils.generate_success_message(inter, Strings.event_data_manager_fetch_data_success(guild=data.name))
