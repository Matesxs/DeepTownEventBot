...
async def create_dummy_dt_guild_member(user_id: int, guild_id: int, event_year: Optional[int] = None, event_week: Optional[int] = None, contribution_amount: Optional[int] = None) -> Optional[DTGuildMember]:
  ...
  if item is None:
    if event_year is not None and event_week is not None and contribution_amount is not None:
      ...
      if not await have_participation_elsewhere(user_id, guild_id, event_year, event_week, contribution_amount):
...
async def get_and_update_dt_guild_members(guild_data: DTGuildData, event_year: Optional[int] = None, event_week: Optional[int] = None) -> Optional[List[DTGuildMember]]:
  ...
  for player_data in guild_data.players:
    ...
    if item is None:
      if event_year is not None and event_week is not None:
        # If this player have this specific event already participation in other guild, dont create him
        if not await have_participation_elsewhere(player_data.id, guild_data.id, event_year, event_week, player_data.last_event_contribution):
