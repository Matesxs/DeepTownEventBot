...
class Common(Base_Cog):
  ...
  async def uptime(self, inter: disnake.CommandInteraction):
    ...
    description = f"{humanize.naturaldelta(datetime.datetime.now(datetime.UTC).replace(tzinfo=None) - self.bot.start_time)}\nLast error: {humanize.naturaltime(datetime.datetime.now(datetime.UTC).replace(tzinfo=None) - self.bot.last_error) if self.bot.last_error is not None else 'Never'}"
