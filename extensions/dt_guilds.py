...
from database import event_participation_repo, dt_guild_repo, dt_guild_member_repo, session_maker
...
class DTGuilds(Base_Cog):
  ...
  async def guild_report(self, inter: disnake.CommandInteraction,
    ...
    with session_maker() as session:
      guild_data = await event_participation_repo.get_event_participations(session, guild_id=identifier[1], year=event_identifier[0], week=event_identifier[1], order_by=[event_participation_repo.EventParticipation.amount.desc()])
      if not guild_data:
  ...
  async def guild_profile(self, inter: disnake.CommandInteraction,
    ...
    guild_profile_lists = []
    with session_maker() as session:
      guild = await dt_guild_repo.get_dt_guild(session, identifier[1])
      if not guild:
      ...
      guild_front_page.add_field(name="Position", value=str(await dt_guild_repo.get_guild_position(session, guild.id)))
      ...
      all_time_total, all_time_average, all_time_median = await event_participation_repo.get_event_participation_stats(session, guild_id=guild.id, ignore_zero_participation_median=True)
      all_time_total_last_year, all_time_average_last_year, all_time_median_last_year = await event_participation_repo.get_event_participation_stats(session, guild_id=guild.id, year=current_year, ignore_zero_participation_median=True)
      ...
      top_event_contributors = await event_participation_repo.get_event_participants_data(session, guild.id, limit=10, ignore_zero_participation_median=True, only_current_members=True, order_by=[sqlalchemy.func.avg(event_participation_repo.EventParticipation.amount).desc()])
      ...
      worst_event_contributors = await event_participation_repo.get_event_participants_data(session, guild.id, limit=10, ignore_zero_participation_median=True, only_current_members=True, order_by=[sqlalchemy.func.avg(event_participation_repo.EventParticipation.amount)])
  ...
  async def guild_leaderboard(self, inter: disnake.CommandInteraction):
    ...
    with session_maker() as session:
      guild_leaderboard_data = await dt_guild_repo.get_guild_level_leaderboard(session)
      ...
      for standing, guild_id, guild_name, level in guild_leaderboard_data:
        member_number = await dt_guild_member_repo.get_number_of_members(session, guild_id)
