...
async def get_guess(lottery_id: int, author_id: int) -> Optional[DTEventItemLotteryGuess]:
  result = await run_query(select(DTEventItemLotteryGuess).filter(DTEventItemLotteryGuess.lottery_id == lottery_id, DTEventItemLotteryGuess.user_id == str(author_id)))
  return result.scalar_one_or_none()

async def get_lottery_guesses(lottery_id: int) -> List[DTEventItemLotteryGuess]:
  result = await run_query(select(DTEventItemLotteryGuess).filter(DTEventItemLotteryGuess.lottery_id == lottery_id))
  return result.scalars().all()

async def make_guess(lottery_id: int, author: disnake.User, items: List[dt_items_repo.DTItem]) -> Optional[DTEventItemLotteryGuess]:
  lotery = await get_event_item_lottery(lottery_id)
  if lotery is None:
    return None
  ...
  guess = await get_guess(lottery_id, author.id)
  ...
  if guess is not None:
    ...
    guess = DTEventItemLotteryGuess(lottery_id=lottery_id, user_id=str(author.id))
    session.add(guess)
    await run_commit()
...
async def get_results(lottery_id: int) -> Optional[Dict[int, List[int]]]:
  ...
  guesses = await get_lottery_guesses(lottery_id)
