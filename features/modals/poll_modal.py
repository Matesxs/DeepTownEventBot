...
from features.views.poll_prompt import PollView
...
class PollModal(disnake.ui.Modal):
  def __init__(self, author: disnake.User, pool_duration_seconds: int):
    ...
    super(PollModal, self).__init__(title="Setup Poll", custom_id="vote_setup", timeout=300, components=components)
  ...
  async def callback(self, interaction: disnake.ModalInteraction) -> None:
    ...
    vote_prompt = PollView(author=self.author, duration_seconds=self.pool_duration, description=interaction.text_values["vote_setup:description"], choices=choices)
