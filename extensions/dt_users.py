...
from database import event_participation_repo, dt_user_repo, session_maker
...
class DTUsers(Base_Cog):
  ...
  async def user_event_participations(self, inter: disnake.CommandInteraction,
    ...
    with session_maker() as session:
      user = await dt_user_repo.get_dt_user(session, identifier[1])
      if not user:
        return await message_utils.generate_error_message(inter, Strings.dt_user_not_found(identifier=identifier[1]))

      all_participations = await event_participation_repo.get_event_participations(session, user_id=user.id)
  ...
  async def user_profile(self, inter: disnake.CommandInteraction,
    ...
    with session_maker() as session:
      ...
      all_time_total, all_time_average, all_time_median = await event_participation_repo.get_event_participation_stats(session, user_id=user.id, ignore_zero_participation_median=True)
      all_time_total_last_year, all_time_average_last_year, all_time_median_last_year = await event_participation_repo.get_event_participation_stats(session, user_id=user.id, year=current_year, ignore_zero_participation_median=True)
      ...
      participation_pages_data = dt_report_generators.generate_participations_page_strings(await event_participation_repo.get_event_participations(session, user_id=user.id))
      for participation_page_data in participation_pages_data:
        participation_page = disnake.Embed(title=f"{string_manipulation.truncate_string(user.username, 200)} event participations", description=f"```\n{participation_page_data}\n```", color=disnake.Color.dark_blue())
        message_utils.add_author_footer(participation_page, inter.author)
