...
class CreateQuestionAndAnswer(disnake.ui.Modal):
  ...
  async def callback(self, interaction: disnake.ModalInteraction) -> None:
    with session_maker() as session:
      if (await questions_and_answers_repo.create_question_and_answer(session, interaction.text_values["q_and_a:question"], interaction.text_values["q_and_a:answer"])) is None:
